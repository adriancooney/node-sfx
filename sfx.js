...
var sfx = {
	 ...
	 */
	play: function(sound, volume, rate) {
		...
		// Rate
		if(rate && config.play.options.rate) command += " " + sfx.format(config.play.options.rate.syntax, { rate: rate });
	...
	say: function(message, voice) {
		...
		var voices = "Agnes,Albert,Alex,Bad News,Bahh,Bells,Boing,Bruce,Bubbles,Cellos,Deranged,Fred,Good News,Hysterical,Junior,Kathy,Pipe Organ,Princess,Ralph,Trinoids,Vicki,Victoria,Whisper,Zarvox".split(",").map(function(v) { return v.toLowerCase(); });
